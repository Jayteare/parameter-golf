...
class Hyperparameters:
    ...
    hmm_hidden_states = int(os.environ.get("HMM_HIDDEN_STATES", 64))
    hmm_chunk_size = int(os.environ.get("HMM_CHUNK_SIZE", 16))
    hmm_lr = float(os.environ.get("HMM_LR", os.environ.get("MARKOV_LR", os.environ.get("HEAD_LR", "0.008"))))
...
# Keep the HMM parameters in fp16 instead of int8 to preserve
# learned distributions that are highly sensitive to quantization noise.
INT8_KEEP_FLOAT_NAME_PATTERNS = ("transition_logits", "emission_logits", "init_logits")
INT8_KEEP_FLOAT_MAX_NUMEL = 131_072
...
def quantize_state_dict_int8(state_dict: dict[str, Tensor]):
    ...
    for name, tensor in state_dict.items():
        ...
        # Also keep HMM parameters in fp16 — they're highly sensitive to
        # quantization noise and the size cost is worth it.
...
class HiddenMarkovLM(nn.Module):
    """Hidden Markov Model LM: H hidden states with transition + emission matrices.

    Instead of a V*V bigram table (1M params), uses H hidden states with:
    - init: H params (initial state distribution)
    - transition: H*H params (state dynamics)
    - emission: H*V params (observation model)
    Total: ~H*(H+V+1) params. With H=64, V=1024: ~70K vs 1M.
    """
    def __init__(self, vocab_size: int, num_hidden: int, logit_softcap: float):
        super().__init__()
        if logit_softcap <= 0.0:
            raise ValueError(f"logit_softcap must be positive, got {logit_softcap}")
        ...
        self.logit_softcap = logit_softcap
        self.init_logits = nn.Parameter(torch.zeros(num_hidden, dtype=torch.float32))
        self.transition_logits = nn.Parameter(torch.randn(num_hidden, num_hidden, dtype=torch.float32) * 0.01)
        self.emission_logits = nn.Parameter(torch.randn(num_hidden, vocab_size, dtype=torch.float32) * 0.01)

    def forward_logits(self, input_ids: Tensor, chunk_size: int = 16) -> Tensor:
        if input_ids.ndim == 1:
            input_ids = input_ids.unsqueeze(0)
        batch_size, seq_len = input_ids.shape

        A = F.softmax(self.transition_logits.float(), dim=-1)
        B = F.softmax(self.emission_logits.float(), dim=-1)
        pi = F.softmax(self.init_logits.float(), dim=-1)
        ...
        # Reshape: (B, seq_len) -> (B * num_chunks, chunk_size)
        chunked_ids = input_ids.reshape(batch_size * num_chunks, chunk_size)
        chunk_batch = chunked_ids.shape[0]

        # Each chunk starts fresh with the prior
        gamma = pi.unsqueeze(0).expand(chunk_batch, -1).clone()

        all_pred_logits = []
        for t in range(chunk_size):
            obs = chunked_ids[:, t].long()
            b_obs = B[:, obs].T
            gamma = gamma * b_obs
            gamma = gamma / (gamma.sum(-1, keepdim=True) + 1e-10)
            pred_state = gamma @ A
            pred_probs = pred_state @ B
            pred_logits = torch.log(pred_probs + 1e-10)
            all_pred_logits.append(pred_logits)
            gamma = pred_state

        # Stack and reshape back: (chunk_batch, chunk_size, V) -> (B, padded_len, V) -> trim
        logits = torch.stack(all_pred_logits, dim=1)  # (chunk_batch, chunk_size, V)
        logits = logits.reshape(batch_size, padded_len, self.vocab_size)
        if pad_len > 0:
...
class GPTMarkovMixLM(nn.Module):
    ...
    def _confidence_gate(self, residual_logits: Tensor, threshold: float, temp: float) -> Tensor:
        probs = F.softmax(residual_logits.float(), dim=-1)
        top_prob = probs.max(dim=-1).values
        gate = torch.sigmoid((top_prob - threshold) / max(temp, 1e-6))
        return gate.unsqueeze(-1)

    def forward_logits(self, input_ids: Tensor) -> Tensor:
        logits = self.gpt.forward_logits(input_ids)
        markov_logits = self.markov.forward_logits(input_ids)
        gate1 = self._confidence_gate(markov_logits, self.markov_gate_threshold, self.markov_gate_temp)
        logits = logits + gate1 * self.markov_mix.to(dtype=torch.float32) * markov_logits
        if self.markov2 is not None:
...
class GPTHMMMixLM(nn.Module):
    # Add GPT logits to HMM residual, with optional hashed order-2 Markov.
    def __init__(
        self,
        vocab_size: int,
        num_layers: int,
        ...
        self.markov = HiddenMarkovLM(vocab_size=vocab_size, num_hidden=num_hidden, logit_softcap=logit_softcap)
        self.markov_mix = nn.Parameter(torch.tensor(float(markov_mix_init), dtype=torch.float32))
        self.markov_gate_threshold = float(markov_gate_threshold)
        self.markov_gate_temp = float(markov_gate_temp)
        self.markov2 = (
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        logits = self.gpt.forward_logits(input_ids)
        markov_logits = self.markov.forward_logits(input_ids, chunk_size=self.hmm_chunk_size)
        gate1 = self._confidence_gate(markov_logits, self.markov_gate_threshold, self.markov_gate_temp)
        logits = logits + gate1 * self.markov_mix.to(dtype=torch.float32) * markov_logits
        if self.markov2 is not None:
            if self.markov2_mix is None:
...
def main() -> None:
    ...
    args = Hyperparameters()
    if args.model_kind not in {"gpt", "markov", "gpt_markov", "gpt_hmm"}:
        raise ValueError(f"Unsupported MODEL_KIND={args.model_kind}; expected 'gpt', 'markov', 'gpt_markov', or 'gpt_hmm'")
    ...
    elif args.model_kind == "gpt_hmm":
        base_model = GPTHMMMixLM(
            vocab_size=args.vocab_size,
            num_layers=args.num_layers,
            model_dim=args.model_dim,
            num_heads=args.num_heads,
            ...
            rope_base=args.rope_base,
            qk_gain_init=args.qk_gain_init,
            num_hidden=args.hmm_hidden_states,
            markov_mix_init=args.markov_mix_init,
            markov_gate_threshold=args.markov_gate_threshold,
            markov_gate_temp=args.markov_gate_temp,
            markov2_buckets=args.markov2_buckets,
            ...
            markov2_gate_threshold=args.markov2_gate_threshold,
            markov2_gate_temp=args.markov2_gate_temp,
            hmm_chunk_size=args.hmm_chunk_size,
        ).to(device).bfloat16()
    else:
        base_model = GPT(
            vocab_size=args.vocab_size,
    ...
    if args.model_kind == "markov":
        ...
        gpt_model = base_model.gpt if args.model_kind in ("gpt_markov", "gpt_hmm") else base_model
        # Optimizer split:
        ...
        if args.model_kind in ("gpt_markov", "gpt_hmm"):
        ...
        if args.model_kind == "gpt_hmm":
            hmm_params = list(base_model.markov.parameters())
            optimizer_hmm = torch.optim.Adam(
                [{"params": hmm_params, "lr": args.hmm_lr, "base_lr": args.hmm_lr}],
                betas=(args.beta1, args.beta2),
                eps=args.adam_eps,
                fused=True,
            )
            optimizers.append(optimizer_hmm)
            if base_model.markov2 is not None:
                optimizer_markov2 = torch.optim.Adam(
                    [{"params": [base_model.markov2.transition_logits], "lr": args.markov2_lr, "base_lr": args.markov2_lr}],
                    betas=(args.beta1, args.beta2),
    ...
    if args.model_kind == "markov":
        ...
        if args.model_kind == "gpt_hmm":
            log0(
                f"hmm_hidden_states:{args.hmm_hidden_states} hmm_chunk_size:{args.hmm_chunk_size} hmm_lr:{args.hmm_lr} "
                f"markov_mix_init:{args.markov_mix_init} "
                f"markov_gate_threshold:{args.markov_gate_threshold} markov_gate_temp:{args.markov_gate_temp}"
            )
    log0(

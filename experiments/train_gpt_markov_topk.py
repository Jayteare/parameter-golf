...
class Hyperparameters:
    ...
    markov_topk = int(os.environ.get("MARKOV_TOPK", 64))
...
class GPTMarkovMixLM(nn.Module):
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        ...
        # Top-k filter: only keep the K most confident Markov predictions
        if self.markov_topk > 0 and self.markov_topk < markov_logits.size(-1):
            topk_vals, topk_idx = markov_logits.topk(self.markov_topk, dim=-1)
            sparse_markov = torch.full_like(markov_logits, float('-inf'))
            sparse_markov.scatter_(-1, topk_idx, topk_vals)
            # Re-zero the -inf values so they don't affect GPT logits when added
            mask = sparse_markov == float('-inf')
            sparse_markov = sparse_markov.masked_fill(mask, 0.0)
            markov_logits = sparse_markov
    ...
    if args.model_kind == "markov":
        ...
        if args.model_kind == "gpt_markov":
            log0(
                ...
                f"markov2_gate_temp:{args.markov2_gate_temp} markov_topk:{args.markov_topk}"
            )
    log0(

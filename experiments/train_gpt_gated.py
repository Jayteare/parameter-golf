...
class GPTMarkovMixLM(nn.Module):
    ...
    def __init__(
        ...
        self.markov_gate_temp = max(float(markov_gate_temp), 1e-6)
        self.markov2 = (
    ...
    @staticmethod
    def _confidence_gate(logits: Tensor, threshold: float, temp: float) -> Tensor:
        probs = torch.softmax(logits.float(), dim=-1)
        top_prob = probs.amax(dim=-1, keepdim=True)
        return torch.sigmoid((top_prob - threshold) / temp).to(dtype=logits.dtype)

    def forward_logits(self, input_ids: Tensor) -> Tensor:
        logits = self.gpt.forward_logits(input_ids)
        markov_logits = self.markov.forward_logits(input_ids)
        markov_gate = self._confidence_gate(markov_logits, self.markov_gate_threshold, self.markov_gate_temp)
        logits = logits + self.markov_mix.to(dtype=torch.float32) * markov_gate * markov_logits
        if self.markov2 is not None:
            ...
            markov2_gate = self._confidence_gate(markov2_logits, self.markov2_gate_threshold, self.markov2_gate_temp)
            logits = logits + self.markov2_mix.to(dtype=torch.float32) * markov2_gate * markov2_logits
        return logits

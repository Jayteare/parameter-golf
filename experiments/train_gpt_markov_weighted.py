...
class Hyperparameters:
    ...
    # Weighted prediction: tiered confidence gates (relaxed/strict/absolute)
    markov_relaxed_threshold = float(os.environ.get("MARKOV_RELAXED_THRESHOLD", 0.10))
    markov_strict_threshold = float(os.environ.get("MARKOV_STRICT_THRESHOLD", 0.30))
    markov_absolute_threshold = float(os.environ.get("MARKOV_ABSOLUTE_THRESHOLD", 0.60))
    markov_relaxed_weight_init = float(os.environ.get("MARKOV_RELAXED_WEIGHT_INIT", 0.02))
    markov_strict_weight_init = float(os.environ.get("MARKOV_STRICT_WEIGHT_INIT", 0.06))
    markov_absolute_weight_init = float(os.environ.get("MARKOV_ABSOLUTE_WEIGHT_INIT", 0.15))
...
class GPTMarkovMixLM(nn.Module):
    # GPT + Markov with weighted prediction gates (relaxed/strict/absolute).
    # Inspired by calibrated prediction networks: tiered confidence gates let
    # the Markov signal contribute proportionally to how certain it is.
    # - Relaxed: low threshold, fires often, small learned weight (hedging)
    # - Strict: medium threshold, fires sometimes, medium weight
    # - Absolute: high threshold, fires rarely, commits hard to Markov
    def __init__(
        self,
        vocab_size: int,
        num_layers: int,
        ...
        relaxed_weight_init: float,
        strict_weight_init: float,
        absolute_weight_init: float,
        markov2_buckets: int,
        ...
        # Tiered weighted prediction gates
        self.gate_temp = max(float(gate_temp), 1e-6)
        ...
        self.w_relaxed = nn.Parameter(torch.tensor(float(relaxed_weight_init), dtype=torch.float32))
        self.w_strict = nn.Parameter(torch.tensor(float(strict_weight_init), dtype=torch.float32))
        self.w_absolute = nn.Parameter(torch.tensor(float(absolute_weight_init), dtype=torch.float32))
        ...
        self.markov2_gate_temp = max(float(markov2_gate_temp), 1e-6)
    ...
    def _confidence_gate(self, residual_logits: Tensor, threshold: float, temp: float) -> Tensor:
        ...
        gate = torch.sigmoid((top_prob - threshold) / temp)
        return gate.unsqueeze(-1)

    def _weighted_prediction(self, markov_logits: Tensor) -> Tensor:
        # Compute confidence from Markov's softmax distribution
        probs = F.softmax(markov_logits.float(), dim=-1)
        confidence = probs.max(dim=-1).values.unsqueeze(-1)

        # Three tiered gates based on appearance-calibrated confidence
        relaxed_gate = torch.sigmoid((confidence - self.relaxed_threshold) / self.gate_temp)
        strict_gate = torch.sigmoid((confidence - self.strict_threshold) / self.gate_temp)
        absolute_gate = torch.sigmoid((confidence - self.absolute_threshold) / self.gate_temp)

        # Blend: low confidence = small nudge, high confidence = strong commitment
        weight = (self.w_relaxed * relaxed_gate
                + self.w_strict * strict_gate
                + self.w_absolute * absolute_gate)
        return weight

    def forward_logits(self, input_ids: Tensor) -> Tensor:
        logits = self.gpt.forward_logits(input_ids)
        markov_logits = self.markov.forward_logits(input_ids)
        weight = self._weighted_prediction(markov_logits)
        logits = logits + weight * markov_logits
        if self.markov2 is not None:
...
def main() -> None:
    ...
    if args.model_kind == "markov":
        ...
        base_model = GPTMarkovMixLM(
            ...
            gate_temp=args.markov_gate_temp,
            relaxed_threshold=args.markov_relaxed_threshold,
            strict_threshold=args.markov_strict_threshold,
            absolute_threshold=args.markov_absolute_threshold,
            relaxed_weight_init=args.markov_relaxed_weight_init,
            strict_weight_init=args.markov_strict_weight_init,
            absolute_weight_init=args.markov_absolute_weight_init,
    ...
    if args.model_kind == "markov":
        ...
        if args.model_kind == "gpt_markov":
            scalar_params.extend([base_model.w_relaxed, base_model.w_strict, base_model.w_absolute])
    ...
    if args.model_kind == "markov":
        ...
        if args.model_kind == "gpt_markov":
            log0(
                f"markov_lr:{args.markov_lr} gate_temp:{args.markov_gate_temp} "
                f"weighted_prediction: relaxed({args.markov_relaxed_threshold}, init={args.markov_relaxed_weight_init}) "
                f"strict({args.markov_strict_threshold}, init={args.markov_strict_weight_init}) "
                f"absolute({args.markov_absolute_threshold}, init={args.markov_absolute_weight_init}) "
                f"markov2_buckets:{args.markov2_buckets}"
            )
    log0(

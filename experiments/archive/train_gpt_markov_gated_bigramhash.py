...
class Hyperparameters:
    ...
    # BigramHash embedding parameters.
    bigram_buckets = int(os.environ.get("BIGRAM_BUCKETS", 0))  # 0 = disabled
...
class BigramHashEmbedding(nn.Module):
    """Hash (prev_token, cur_token) into buckets, look up learned embedding, project to model_dim."""
    ...
    def forward(self, input_ids: Tensor) -> Tensor:
        ...
        cur = input_ids.to(dtype=torch.int64)
        prev = torch.roll(cur, shifts=1, dims=1)
        prev[:, 0] = 0
        hashed = ((prev * 92821) + cur) % self.num_buckets
        emb = self.embed(hashed)
...
def main() -> None:
    ...
    if args.model_kind == "markov":
        ...
        if gpt_model.bigram is not None:
            bigram_params = list(gpt_model.bigram.parameters())

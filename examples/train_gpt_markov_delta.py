...
def quantize_state_dict_int8(state_dict: dict[str, Tensor]):
    ...
    for name, tensor in state_dict.items():
        ...
        delta_encoded = False
        if MARKOV_DELTA_ENABLE and t.ndim == 2 and any(pattern in name for pattern in MARKOV_DELTA_NAME_PATTERNS):
            # Row-wise delta coding along the vocabulary axis often makes transition
            # tables locally smoother and more compressible after int8+zlib.
            t = torch.diff(t, dim=1, prepend=t[:, :1]).contiguous()
            delta_encoded = True
        ...
        if t.numel() <= INT8_KEEP_FLOAT_MAX_NUMEL:
            ...
            if delta_encoded:
                qmeta[name] = {"scheme": "delta_passthrough_row", "axis": 1}
        ...
        if delta_encoded:
            qmeta[name] = {"scheme": "delta_per_row", "axis": 1}
        elif s.ndim > 0:
...
def dequantize_state_dict_int8(obj: dict[str, object]) -> dict[str, Tensor]:
    ...
    for name, q in obj["quantized"].items():
        ...
        if scheme in {"per_row", "delta_per_row"} or s.ndim > 0:
    ...
    for name, t in obj["passthrough"].items():
        ...
        if qmeta.get(name, {}).get("scheme") == "delta_passthrough_row":
            out_t = torch.cumsum(out_t, dim=1).contiguous()
        out[name] = out_t
    return out
